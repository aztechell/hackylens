...
APP_INCLUDE_DIRS = {
    app_id: tuple([
        directory,
        *(directory / include for include in APP_MANIFESTS[app_id]["private_includes"]),
    ])
    for app_id, directory in APP_SOURCE_DIRS.items()
}
...
def app_include_sets(
    stage: Path,
    disabled_apps: set[str],
) -> tuple[tuple[Path, ...], tuple[Path, ...]]:
    allowed = tuple(sorted({
        stage / include
        for app_id, includes in APP_INCLUDE_DIRS.items()
        if app_id not in disabled_apps
        for include in includes
    }))
    app_root = stage / "firmware" / "src" / "apps"
    implicit = {
        header.parent
        for header in app_root.rglob("*.h")
        if header.is_file()
    } if app_root.is_dir() else set()
    return allowed, tuple(sorted(implicit - set(allowed)))
...
def write_project_cmake(
    ...
    allowed_app_includes, forbidden_app_includes = app_include_sets(
        stage, disabled_apps or set()
    )
    lines = [
        "# Generated by tools/build_firmware.py",
        "# The legacy SDK recursively promotes every header directory to -I.",
        "# Remove accidental roots, then add only manifest-authorized app roots.",
        "set(_HACKYLENS_CAPABILITY_LEAF \"${CMAKE_CURRENT_LIST_DIR}/firmware/include/hackylens/capability\")",
        "set(_HACKYLENS_FORBIDDEN_APP_INCLUDE_DIRS",
        *(f'    "{cmake_path(path)}"' for path in forbidden_app_includes),
        ")",
        "function(_hackylens_prune_implicit_includes directory)",
        ...
        "            list(REMOVE_ITEM _includes ${_HACKYLENS_FORBIDDEN_APP_INCLUDE_DIRS})",
        ...
        "        _hackylens_prune_implicit_includes(\"${_subdirectory}\")",
        "    endforeach()",
        "endfunction()",
        "_hackylens_prune_implicit_includes(\"${SDK_ROOT}\")",
