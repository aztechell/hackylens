...
def compose(
    ...
    *,
    ...
    def evaluate(name: str, requirements: Requirements, *, app: bool) -> None:
        ...
        if failures:
            if not app:
                # The scheduler calls the static Time binding directly. A
                # diagnostic profile cannot omit it while retaining runtime.
                missing_time = any(TIME_SERVICE_ID in failure["missing"]
                                   for failure in failures)
                if allow_required_consumer_exclusion and not missing_time:
                    first = failures[0]
...
def capabilities_document(composition: Composition) -> dict[str, object]:
    entries = []
    for item in runtime_capabilities(composition):
        entries.append({
            "id": item.id,
...
def composition_document(composition: Composition, capabilities_sha256: str) -> dict[str, object]:
    return {
        ...
        "capabilities": {
            ...
            "entry_count": len(runtime_capabilities(composition)),
        },
        "grants": {
...
def generated_c(composition: Composition) -> str:
    capabilities = runtime_capabilities(composition)
    lines = [
    ...
    if capabilities:
        ...
        lines.extend(f"    &{item.provider_symbol}," for item in capabilities)
        lines.extend(["};", ""])
    ...
    if capabilities:
        lines.extend([
            ...
            f"    if(count)", f"        *count = {len(capabilities)}U;",
        ])
    else:
        lines.extend([
