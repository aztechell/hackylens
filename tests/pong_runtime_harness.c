...
#include "../firmware/src/apps/pong/pong_app.h"
...
int main(void)
{
    ...
    hk_app_runtime_host_fill_app(&app, "pong", &pong_v2_entry, 1024U);
    ...
    CHECK(hk_app_switch_poll(switcher, hk_app_runtime_host_now_us(&host)) ==
          HK_OK);
    CHECK(host.present_calls == 1U);
    ...
    hk_app_runtime_host_fill_app(&app, "pong", &pong_v2_entry, 1024U);
    {
        ...
        hk_app_runtime_host_fill_app(
            &buttons, "buttons", &buttons_v2_entry, 1024U);
        switcher = hk_app_runtime_host_switch(&host);
        CHECK(hk_app_switch_open(switcher, &buttons, NULL) == HK_OK);
        CHECK(hk_app_switch_open(switcher, &app, NULL) == HK_OK);
        CHECK(hk_app_switch_active(switcher) == &app);
        CHECK(hk_app_switch_open(switcher, &buttons, NULL) == HK_OK);
        CHECK(hk_app_switch_active(switcher) == &buttons);
        CHECK(hk_app_switch_open(switcher, &app, NULL) == HK_OK);
        CHECK(dispatch_input(&host, HK_INPUT_BUTTON_BACK, &consumed) == 0);
        CHECK(hk_app_switch_active(switcher) == NULL);
        CHECK(hk_app_switch_open(switcher, &buttons, NULL) == HK_OK);
