...
class TerminalAppTests(unittest.TestCase):
    def test_scroll_back_reopen_and_font_hold(self) -> None:
        compiler = shutil.which("gcc") or shutil.which("cc")
        if not compiler:
            raise unittest.SkipTest("host C compiler not installed")

        with tempfile.TemporaryDirectory(prefix="hackylens-terminal-") as temp:
            executable = Path(temp) / (
                "terminal_controller.exe" if os.name == "nt" else "terminal_controller"
            )
            subprocess.run(
                [
                    compiler,
                    ...
                    f"-I{ROOT / 'firmware' / 'src' / 'apps' / 'terminal'}",
                    f"-I{ROOT / 'firmware' / 'assets'}",
                    str(ROOT / "tests" / "terminal_controller_harness.c"),
                    str(
                        ROOT
                        / "firmware"
                        / "src"
        ...
        self.assertIn(
            "TERMINAL_CONTROLLER_OK back=1 reopen=1 font_hold=1",
            result.stdout,
        )
    ...
    def test_production_runtime_delivers_back_and_stop_cleanup(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="hackylens-terminal-runtime-") as temp:
            executable = Path(temp) / (
                "terminal_runtime.exe" if os.name == "nt" else "terminal_runtime"
            )
            subprocess.run(
                [
                    compiler,
                    ...
                    f"-I{ROOT / 'firmware' / 'assets'}",
                    f"-I{ROOT / 'tests'}",
                    str(ROOT / "tests" / "terminal_runtime_harness.c"),
                    ...
                    str(ROOT / "firmware" / "src" / "apps" / "terminal" / "terminal_view.c"),
                    str(ROOT / "firmware" / "src" / "ui" / "hk_font.c"),
                    str(ROOT / "firmware" / "src" / "core" / "hk_log.c"),
                    "-o",
                    str(executable),
                ],
                check=True,
        ...
        self.assertIn("TERMINAL_RUNTIME_OK", result.stdout)
