...
{
    hk_display_t base = {0};
    hk_display_t copied = {0};
    hk_display_t other_base = {0};
    hk_display_t overlay = {0};
    hk_display_info_t info;

    hk_fake_display_reset(HK_DISPLAY_PLANE_ALL);

    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    copied = base;
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &other_base) == HK_ERR_BUSY);
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_OVERLAY, &overlay) == HK_OK);
    CHECK(hk_display_get_info(&copied, &info) == HK_ERR_STALE_HANDLE);
    CHECK(hk_display_get_info(&base, &info) == HK_OK);
    ...
    CHECK(hk_display_close(&overlay, HK_DEADLINE_IMMEDIATE) == HK_OK);
    CHECK(hk_display_close(&base, HK_DEADLINE_IMMEDIATE) == HK_OK);
    CHECK(hk_display_get_info(&copied, &info) == HK_ERR_STALE_HANDLE);
    CHECK(hk_display_close(&base, HK_DEADLINE_IMMEDIATE) == HK_OK);

    hk_fake_display_reset(HK_DISPLAY_PLANE_BASE);
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_OVERLAY, &overlay) ==
...
static int test_clipping_noops_and_log(void)
{
    ...
    hk_fake_display_reset(HK_DISPLAY_PLANE_ALL);
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_set_clip(&base, &clip) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &partial, 0xF800U) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &outside, 0U) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &empty, 0U) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &overflow, 0U) ==
    ...
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, NULL) == HK_OK);
    metrics = hk_fake_display_metrics();
    ...
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, NULL) ==
...
static int test_transactional_limits(void)
{
    ...
    for(uint32_t index = 0U; index < HK_FAKE_DISPLAY_MAX_COMMANDS; index++)
        CHECK(hk_display_fill_rect(&base, &same, 0U) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &same, 0U) == HK_ERR_LIMIT);
    metrics = hk_fake_display_metrics();
    CHECK(metrics->staged_commands == HK_FAKE_DISPLAY_MAX_COMMANDS &&
          metrics->staged_dirty_rects == 1U);
    CHECK(hk_display_abort(&base) == HK_OK);

    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_text(&base, &text_bounds, text, sizeof(text), 0xFFFFU) == HK_OK);
    CHECK(hk_display_text(&base, &text_bounds, "y", 1U, 0xFFFFU) == HK_ERR_LIMIT);
    metrics = hk_fake_display_metrics();
    ...
    for(uint32_t index = 0U; index < HK_FAKE_DISPLAY_MAX_DIRTY_RECTS; index++)
    {
        ...
        CHECK(hk_display_mark_dirty(&base, &dirty) == HK_OK);
    }
    same.x = 8;
    CHECK(hk_display_mark_dirty(&base, &same) == HK_ERR_LIMIT);
    CHECK(hk_fake_display_metrics()->staged_dirty_rects ==
          HK_FAKE_DISPLAY_MAX_DIRTY_RECTS);
    CHECK(hk_display_abort(&base) == HK_OK);

    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_clear(&base, 0U) == HK_OK);
    hk_fake_display_set_slice_duration_us(20000U);
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, NULL) == HK_ERR_LIMIT);
...
static int test_cancel_retry_and_deadline(void)
{
    ...
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &dirty, 0x07E0U) == HK_OK);
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, &cancel) ==
          HK_ERR_CANCELLED);
    metrics = hk_fake_display_metrics();
    CHECK(metrics->transferred_bytes == 8U &&
    ...
    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &dirty, 0U) == HK_OK);
    hk_fake_display_set_now_us(600000U);
    bytes = hk_fake_display_metrics()->transferred_bytes;
    CHECK(hk_display_present(&base, (hk_deadline_t){600000U}, NULL) ==
...
static int test_surface_blit_and_incremental_bytes(void)
{
    ...
    hk_fake_display_reset(HK_DISPLAY_PLANE_ALL);
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    CHECK(hk_display_surface_acquire(&base, &surface) == HK_OK);
    ...
    CHECK(hk_display_blit(&base, &destination, &invalid,
        HK_DISPLAY_FORMAT_RGB565_BE) == HK_ERR_INVALID_ARGUMENT);
    invalid = view;
    invalid.size_bytes--;
    CHECK(hk_display_blit(&base, &destination, &invalid,
        HK_DISPLAY_FORMAT_RGB565_BE) == HK_ERR_INVALID_ARGUMENT);
    CHECK(hk_display_blit(&base, &destination, &view,
    ...
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, NULL) == HK_ERR_IO);
...
static int test_disjoint_damage_stays_bounded_during_repair(void)
{
    ...
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &first, 0U) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &last, 0U) == HK_OK);
...
static int test_partial_failure_abort_cleanup(void)
{
    ...
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_BASE, &base) == HK_OK);
    CHECK(hk_display_begin_batch(&base) == HK_OK);
    CHECK(hk_display_fill_rect(&base, &dirty, 0x001FU) == HK_OK);
    hk_fake_display_fail_next_present(HK_ERR_IO, 1U);
    CHECK(hk_display_present(&base, (hk_deadline_t){500000U}, NULL) == HK_ERR_IO);
    CHECK(hk_fake_display_metrics()->transferred_bytes == 8U &&
          hk_fake_display_metrics()->needs_repair_planes ==
    ...
    hk_fake_display_reset(HK_DISPLAY_PLANE_ALL);
    CHECK(hk_display_open(hk_display_service(), HK_DISPLAY_PLANE_OVERLAY, &overlay) == HK_OK);
    CHECK(hk_display_begin_batch(&overlay) == HK_OK);
    CHECK(hk_display_clear(&overlay, 0U) == HK_OK);
    CHECK(hk_display_present(&overlay, (hk_deadline_t){500000U}, NULL) == HK_OK);
    before = hk_fake_display_metrics()->transferred_bytes;
    CHECK(before == HK_FAKE_DISPLAY_WIDTH * HK_FAKE_DISPLAY_HEIGHT * 2U);
    CHECK(hk_display_close(&overlay, (hk_deadline_t){48000U}) ==
          HK_ERR_DEADLINE_EXCEEDED);
...
int main(void)
{
    ...
    puts("DISPLAY_CONTRACT_OK cases=16 normative=8 full_bytes=384 slice_bytes=8");
    return 0;
}
