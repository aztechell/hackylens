...
watchdog не выполняются. Причина WDT1 сохраняется, firmware autostart и
автоматический запуск startup script подавляются на этот boot, чтобы не получить
boot loop; явный ручной RUN остаётся доступен.
...
- после watchdog reset startup script подавляется на один boot и показывается
  причина предыдущего аварийного завершения.

Startup script не должен запускаться из общего boot orchestration напрямую.
Сохраняется autostart ID приложения, приложение входит обычным lifecycle и уже
внутри запускает выбранный script после короткого отменяемого countdown.
