...
- Feature views and controllers add no duplicate framebuffer. CAMERA/QR/FACE/APRILTAG share the camera frame pool and preview surface; menu icons live in feature views as code/constant data. FILES disposal mode 3 continues to reuse the existing camera scratch slot.
- The shared AI runtime allocates only the active model plus the KPU SDK's
  ...
  The optional SD manifest is read through one 256-byte stack buffer. FACE
  retains its existing 230,400-byte aligned planar input and small YOLO
  candidate/result arrays; moving lifecycle code to the runtime adds no second
  model or input buffer.

The settings record grows from the legacy 16-byte payload through the 96-byte v2 payload to a 97-byte v3 payload containing the fixed 80-byte opaque app block and one autostart ID. The aligned 116-byte record still fits in a single flash page. Build guard: `hackylens.bin` must remain below `0x007FE000`, preserving the two 4 KB settings slots at `0x007FE000` and `0x007FF000`.
